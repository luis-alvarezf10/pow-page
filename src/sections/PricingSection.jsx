...
export default function PricingSection() {
  const plans = [
    ...
    },
    {
      ...
      period: '/persona/mes',
      icon: Users,
      features: ['2+ personas', 'Descuento por familia', 'Variación de entrenamientos'],
    },
    {
      name: 'Plan Personal',
      price: '$120',
      period: '/mes',
      icon: Target,
      features: ['Horario Flexible', 'Coach Personal', 'Consejos de Alimentación'],
    }
  ];
    ...
    <section id="pricing" className="relative bg-gray-900 py-24 overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8 relative z-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {plans.map((plan, index) => (
            <div 
              key={index} 
              className={`backdrop-blur-md border rounded-xl p-6 relative ${
                plan.popular ? 'bg-white/10 border-yellow-400 ring-1 ring-yellow-400/20 ' : 'bg-white/5 border-white/20'
              }`}
            >
              {plan.popular && (
              ...
              <div className="text-center mb-6">
                ...
                <h3 className="text-2xl font-semibold text-yellow-400 mb-2">{plan.name}</h3>
              ...
              <button className={`w-full py-3 rounded-lg font-semibold transition-colors cursor-pointer ${
                plan.popular 
                  ? 'bg-yellow-400 text-black hover:bg-yellow-300 shadow-[0_0_10px_rgba(250,204,21,0.4)]' 
                  : 'bg-transparent text-white hover:bg-white/5 border border-yellow-400'
              }`}>
                Selecionar Plan
              </button>
            </div>
      ...
      <div className="px-6">
        <div className="flex flex-col md:flex-row bg-white/5 backdrop-blur-md border border-white/20 rounded-2xl shadow-lg p-6 gap-6 w-full md:w-200 mx-auto my-10 px-10 z-20">
          {/* Precio */}
          <div className="flex flex-col items-center justify-center bg-gradient-to-b from-yellow-400/20 to-transparent rounded-xl p-4 min-w-[120px]">
          ...
          <div className="flex-1 ">
            <h3 className="text-4xl font-bold text-yellow-400 mb-1 text-center">Plan Anual</h3>
            <span className="block text-md text-gray-300 mb-4">
              Ahorra hasta un 16.67% con nuestro plan anual
            </span>
            <ul className="space-y-2 text-white/90 list-disc list-inside list-none text-md">
              <li><span className="text-yellow-400 mr-2">✓</span>Todas las clases incluidas</li>
              <li><span className="text-yellow-400 mr-2">✓</span>Evaluación Física Semestral</li>
              <li><span className="text-yellow-400 mr-2">✓</span>2 consultas nutricionales gratis</li>
            </ul>
          </div>

          {/* Acción */}
          <div className="flex flex-col items-center justify-center gap-2">
            <button className="bg-yellow-400 w-full hover:bg-yellow-300 text-black font-semibold py-3 px-10 rounded-lg shadow-[0_0_12px_rgba(250,204,21,0.4)] transition cursor-pointer relative z-10">
              ¡Ahorra $120!
            </button>
