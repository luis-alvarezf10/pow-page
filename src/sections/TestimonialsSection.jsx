...
const TestimonialsSection = () => {
  ...
  return (
    <section id="testimonials" className="relative bg-gray-900 py-24 overflow-hidden">
      ...
      >
        {[...testimonials, ...testimonials, ...testimonials].map((testimonial, index) => (
          <div key={index} className="w-96 flex-shrink-0">
            <div className="testimonial-card bg-gray-800/50 backdrop-blur-sm p-8 rounded-3xl border border-gray-700 text-center h-80 flex flex-col justify-center transition-all duration-300">
              <Quote className="w-12 h-12 text-yellow-400 fill-current mx-auto mb-6" />
              <p className="text-gray-200 mb-6 text-lg leading-relaxed italic">
                "{testimonial.text}"
              </p>
              <div className="font-semibold text-yellow-400 text-lg">{testimonial.name}</div>
            </div>
          </div>
        ))}
      </div>
